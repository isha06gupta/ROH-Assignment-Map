...
joblib.dump(df_wagon_curr_holding,"df_wagon_curr_holding.pkl")
...
joblib.dump(df_depo_capacity,"df_depo_capacity.pkl")
...
for b in df_wagon_type_comb['depot'].unique():
    try:
        ...
        prob += (
            ...
            "Sum_of_wagons_assigned_should_be_less_than_capacity"+"%s" % b,) 
'''for w in wgnid_loc_wgntype:
    for b in Workdepots:
        try:
            prob+= (
            (vars[w][b]) <= Wagon_handled[b][w[4:7]],
            "wagon_handled_current_either_zero_or_one"+str(b)+"%s" % w,
    )
        except:
            prob+= (
            (vars[w][b]) <= Wagon_handled[b][w[5:8]],
            "wagon_handled_current_either_zero_or_one"+str(b)+"%s" % w,
    )'''
for w in df_wagon_due_roh['wagon_var'].values:
...
wagon_locn_dict    
'''location=Wagon_data['location'].unique()
for c in location:
    for type in Wagon_type:
        try:
            prob += (
            lpSum([vars[w][b]  for w in wgnid_loc_wgntype if type in w if 'L_'+str(c) in w for b in Workdepots]) <= wagon_locn_dict[c][type],
...
for i in df_wagon_type_comb['depot'].unique():
    try:
        ...
        sum_1=sum_1+max(Workdepot_capcity[b]-0,0)
sum_2=0
